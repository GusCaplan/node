...
function getDataURISource(url) {
  const { URL } = require('url');
  const { Buffer } = require('buffer');

  const parsedURL = new URL(url);
  const match = RegExpPrototypeExec(DATA_URL_PATTERN, parsedURL.pathname);
  if (!match) {
    throw new ERR_INVALID_URL(url);
  }
  const { 1: mediaType, 2: base64, 3: body } = match;
  return {
    mediaType,
    data: Buffer.from(decodeURIComponent(body), base64 ? 'base64' : 'utf8'),
  };
}
