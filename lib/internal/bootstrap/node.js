...
if (!config.noBrowserGlobals) {
  ...
  ['Headers', 'Request', 'Response', 'fetch'].forEach((name) => {
    defineLazyGlobal(globalThis, name, () => {
      const fetchImpl = require('internal/fetch');
      return fetchImpl[name];
    });
  });

  const { setWasmStreamingCallback } = internalBinding('wasm_streaming');

  const handleStreaming = async (response, streaming, handler) => {
    if (response[SymbolToStringTag] !== 'Response') {
      // eslint-disable-next-line no-restricted-syntax
      throw new TypeError(
        'Expected Response or Promise resolving to Response');
    }
    const mimeType = response.headers.get('content-type');
    if (mimeType !== 'application/wasm') {
      // eslint-disable-next-line no-restricted-syntax
      throw new TypeError(`Response has unsupported MIME type '${mimeType}'`);
    }
    if (!response.ok) {
      // eslint-disable-next-line no-restricted-syntax
      throw new TypeError('Response returned non-ok status');
    }
    streaming.setURL(response.url);
    if (handler?.get) {
      const result = await handler.get(response.url);
      if (result !== undefined && result !== null) {
        if (!ArrayBufferIsView(result)) {
          // eslint-disable-next-line no-restricted-syntax
          throw new TypeError(
            'Expected an ArrayBufferView from handler.get');
        }
        if (!streaming.setCompiledModuleBytes(result)) {
          handler.delete(response.url);
        }
      }
    }
    for await (const chunk of response.body) {
      streaming.onBytesReceived(chunk);
    }
    streaming.finish();
  };

  setWasmStreamingCallback((response, streaming, handler) => {
    PromisePrototypeCatch(
      handleStreaming(response, streaming, handler),
      (e) => streaming.abort(e));
  });
