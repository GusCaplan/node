...
(function bootstrapNodeJSCore(process,
  ...
  function startup() {
    ...
    {
      let lazyJSStreamWrap;
      let lazyReadableStream;
      let lazy_ERR_INVALID_ARG_TYPE;
      _setupWasmStreaming((source) => {
        if (lazyReadableStream === undefined) {
          lazyReadableStream = NativeModule.require('stream').Readable;
        }

        if (typeof source === 'object' &&
            source instanceof lazyReadableStream) {
          if (lazyJSStreamWrap === undefined) {
            lazyJSStreamWrap = NativeModule.require('internal/wrap_js_stream');
          }
          return new lazyJSStreamWrap(source)._handle._externalStream;
        }

        if (lazy_ERR_INVALID_ARG_TYPE === undefined) {
          lazy_ERR_INVALID_ARG_TYPE = NativeModule.require('internal/errors')
            .codes.ERR_INVALID_ARG_TYPE;
        }

        throw new lazy_ERR_INVALID_ARG_TYPE(
          'source', ['ArrayBuffer View', 'Buffer', 'ReadableStream'], source);
      });
    }
