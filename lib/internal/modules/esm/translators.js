...
const { ERR_INVALID_MIME_TYPE } = require('internal/errors').codes;
...
translators.set('data_url', async (url) => {
  debug(`Translating Data URL ${url}`);
  const { mimeType, body } = internalURLModule.parseDataURL(url);
  if (mimeType !== 'text/javascript') {
    throw new ERR_INVALID_MIME_TYPE(mimeType, 'text/javascript');
  }
  return {
    module: new ModuleWrap(stripShebang(body), url),
    reflect: undefined,
  };
});
