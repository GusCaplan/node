...
const {
  setWasmStreamingCacheHandler: _setWasmStreamingCacheHandler,
} = internalBinding('wasm_streaming');
...
function setWasmStreamingCacheHandler(handler) {
  if (handler === undefined || handler === null) {
    _setWasmStreamingCacheHandler(null, null);
  } else {
    ...
    _setWasmStreamingCacheHandler(handler, (url, buf) => {
      try {
        handler.set(url, new Uint8Array(buf));
      } catch (e) {
